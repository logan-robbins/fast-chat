                    ...
                    if line.startswith("event: "):
                        ...
                        if current_event_type and current_event_type != "message":
                            ...
                            try:
                                ...
                                if current_event_type == "token":
                                    content = _extract_text_content(event_data.get("content", ""))
                                    if content:
                        ...
                        try:
                            ...
                            if chunk_data.get("type") == "token":
                                content = _extract_text_content(chunk_data.get("content", ""))
                                if content:
                                    full_content.append(content)
                                    completion_tokens += count_tokens(content, model)
...
def _extract_text_content(value: Any) -> str:
    """Normalize backend message content to plain text."""
    if value is None:
        return ""
