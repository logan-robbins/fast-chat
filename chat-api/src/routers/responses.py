            ...
            if isinstance(item, dict):
                ...
                if isinstance(content, str):
                    ...
                    for c in content:
                        if isinstance(c, dict) and c.get("type") == "input_text":
                            texts.append(c.get("text", ""))
                        elif hasattr(c, "type") and getattr(c, "type", None) == "input_text":
...
def extract_text_content(value: Any) -> str:
    """Normalize backend message/delta content payloads to plain text."""
    if value is None:
        return ""
                    ...
                    if line.startswith("event: "):
                        ...
                        try:
                            ...
                            if chunk_data.get("type") == "token":
                                content = extract_text_content(chunk_data.get("content", ""))
                                if content:
