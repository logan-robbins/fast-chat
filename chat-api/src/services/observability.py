...
def record_metric(name: str, latency_ms: float, success: bool) -> None:
    """Track an SLO metric (count, latency, failure) keyed by `name`."""
    m = _metrics[name]
    m.count += 1
    if not success:
        m.failures += 1
...
def get_metric_snapshot() -> dict[str, dict[str, float]]:
    """Return a floating-point snapshot of all recorded SLO metrics."""
...
def emit_audit_event(event_type: str, **payload: Any) -> None:
    """Append a timestamped audit record, discarding the oldest when the buffer fills."""
...
def should_inject_failure(operation: str) -> bool:
    """Check whether the named resilience failpoint is enabled via env var."""
