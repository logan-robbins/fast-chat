    ...
    try:
        ...
        async for mode, chunk in graph.astream(inputs, config, stream_mode=stream_modes):
            if mode == "messages":
                ...
                # SKIP tool call chunks completely to prevent raw JSON leakage
                # If the chunk has tool calls (or tool_call_chunks), we should not treat it as text content
                if hasattr(msg_chunk, "tool_calls") and msg_chunk.tool_calls:
                    continue
                if hasattr(msg_chunk, "tool_call_chunks") and msg_chunk.tool_call_chunks:
                    continue
                    
                chunk_text = content_to_text(msg_chunk.content)
                if chunk_text:
                    # Track completion tokens
                    if include_usage:
                        from chat_app.summarization import count_tokens as _count_tokens
                        completion_tokens += _count_tokens(chunk_text)
        ...
        if messages:
            ...
            for msg in reversed(messages):
                if isinstance(msg, AIMessage) and msg.content:
                    assistant_content = content_to_text(msg.content)
                    break
    ...
    try:
        ...
        async for mode, chunk in graph.astream(resume_command, config, stream_mode=stream_modes):
            if mode == "messages":
                ...
                # Skip ToolMessages - they are handled via status events
