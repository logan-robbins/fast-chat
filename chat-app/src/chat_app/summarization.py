...
def _content_to_text(content: Any) -> str:
    """Normalize structured model content payloads to plain text."""
    if content is None:
        return ""
...
async def summarize_messages(
    ...
    model = init_openai_chat_model("gpt-4o-mini", temperature=0.0, streaming=False)
...
def count_tokens(text: Any) -> int:
    ...
    Last Grunted: 02/04/2026 06:30:00 PM PST
    """
    normalized_text = _content_to_text(text)
    if not normalized_text:
        return 0
    
    if TIKTOKEN_AVAILABLE and _encoding:
        return len(_encoding.encode(normalized_text))
