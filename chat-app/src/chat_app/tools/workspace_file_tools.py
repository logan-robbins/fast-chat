...
def create_workspace_file_tools(workspace: Path | str) -> Sequence[BaseTool]:
    ...
    class _WorkspacePathMixin:
        def get_relative_path(self, file_path: str):  # type: ignore[override]
            """Normalize the provided path before delegating to the base tool.

            This override ensures the relative path is sanitized against the
            workspace root, stripping redundant prefixes and blocking traversal
            attempts before calling the upstream tool helper.
            """
