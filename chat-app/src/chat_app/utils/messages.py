...
def content_to_text(content: object) -> str:
    ...
    Handles lists of content blocks (dicts or objects) and extracts
    the text content from them.
    
    Args:
        content: String, list, or object representing message content.
    ...
    if isinstance(content, list):
        ...
        for item in content:
            ...
            # Handle object with .text attribute (like AIMessageChunk)
    ...
    # Only return string if it doesn't look like JSON structure?
    # No, fallback to str() if we can't parse it as list/dict/obj
    # But wait, if str(content) yields raw JSON string, that's what we want to AVOID.
    # So fallback to empty string if not text-like?
    return ""
