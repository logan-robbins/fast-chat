...
def _oauth_provider_configured() -> bool:
    ...
    return any(
        (os.getenv(client_id_key, "").strip() and os.getenv(client_secret_key, "").strip())
        for client_id_key, client_secret_key in provider_pairs
    )
...
if _oauth_provider_configured():
    cl.oauth_callback(oauth_callback)
    logger.info("oauth.enabled")
else:
    logger.info("oauth.disabled no providers configured")
...
async def _handle_status_event(
    ...
    """
    # Derive a human-friendly step name and type
    step_type = "tool"
    if status.type == "agent_handoff":
        step_name = f"Agent: {status.agent}" if status.agent else "Agent Handoff"
    elif status.type == "tool_start":
        step_name = status.tool or "Tool"
        if status.tool == "web_search":
            step_name = "Browsing the Web"
        elif status.tool == "search_document_content":
            step_name = "Reading Documents"
    elif status.type == "thinking":
        step_name = "Thinking"
        step_type = "run"  # distinct icon/style for reasoning
    elif status.type == "error":
        step_name = "Error"
    else:
        step_name = "Processing"

    # All steps as "tool" type (mostly) so they are visible with cot="tool_call"
    if current_step is None:
        current_step = cl.Step(name=step_name, type=step_type)
        await current_step.send()

    # Update output - for thinking/tools, this is the "log" content
...
def _format_usage_markdown(usage: UsageEvent) -> str:
    ...
    if usage.context_utilization_pct is not None:
        ...
        lines.append(f"> Context: **{pct:.1f}%** used{warning}")
                ...
                async for sse in event_source.aiter_sse():
                    ...
                    # ---- Content streaming (default event type) ----
                        ...
                        if choices:
                            ...
                            # Ensure content is a string and not empty before streaming
